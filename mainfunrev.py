...
def mainfun(p, n, c, nsim, factor,eta,  reptime):
    ...
    def mcmcinteg1d(w0, x, outx):
        ...
        dom = np.exp(-1/2 *kernel)
        expect = np.sum(upper)/np.sum(dom + 1e-16)
        return expect

    k = c#int(np.round(1/c * (n/np.log(p)) ** (1/2 - 0.01)))
    beta0 = np.zeros(p)
    ...
    for outitr in range(nsim):            
        u =np.random.standard_t(2, n* p)/np.sqrt(2)# (np.random.gamma(2, 2, n* p) -4) / (2 * np.sqrt(2))#np.random.multivariate_normal(np.zeros(p), omega, n)#
        u.resize((n, p))
        u = np.matmul(u, sqrtomega)
        ...
        print(regcalbeta[outitr, :])
        betaini = regcalbeta[outitr, :]
        betaini[:] = 0
        nitr = reptime
        mbeta = np.zeros(nitr)
        ...
        for j in range(0, nitr):
            ...
            #lam = np.min(eta[:np.min([n, p])]) * (np.log(p)/n)**(1/4) * 1
            #eta =  np.max(eta)
            lam =  (np.log(p)/n)**(1/4) * factor#* 1e4 #np.exp(b0 * np.sqrt(v[1])) * v[1]
            eta = eta
    ...
    mseres = np.median(np.sqrt(np.sum(np.abs(resbeta - beta0[np.newaxis, :])**2, 1)), 0)
    msenaive = np.median(np.sqrt(np.sum(np.abs(naivebeta - beta0[np.newaxis, :])**2, 1)), 0)
    mseregcal = np.median(np.sqrt(np.sum(np.abs(regcalbeta - beta0[np.newaxis, :])**2, 1)), 0)
    res = np.array([resbeta, naivebeta,regcalbeta, beta0])
    print([mseres, msenaive, mseregcal])
    return res
